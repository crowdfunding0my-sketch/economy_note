...
SHORTLIST_SIZE = 150  # Stage C（Woodstock fundamentals）に回す候補数の上限
# 2026-09-26: 60→150に拡大。以前は「小型株+PER15以下+3期連続増収増益」という厳しい条件で
# Stage C通過0件だったが、方針転換後の「成長モメンタム」基準（直近期の売上高成長のみ、
# 時価総額150億ドル以下）では60件中15件（約25%）が通過した。ユーザーから「1年で100銘柄
# くらい紹介したい（現状15銘柄では少ない）」との要望を受け、同じ通過率が続くなら
# 150件中35〜40件程度まで最終候補を増やせる計算になる。時価総額の上限（150億ドル）は
# 現行15銘柄の最大が114億ドル(TTMI)でまだ余裕があり、ボトルネックになっていなかったため
# そのままにしている。
