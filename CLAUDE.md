...
### ニュースリンクの一行要約（2026-09-22追加）

「最近のニュース」欄の各リンクに、クリックする前に内容がわかる一行要約を添えるようにした
（[src/formatters/us_stock_rotation.py](src/formatters/us_stock_rotation.py)の`generate_teaser()`）。

- 当初案：Claude（Haiku）にタイトル・要約から「クリックを誘う煽り文」を生成させる方式で実装したが、
  Anthropic APIのクレジット残高不足で利用できず、ユーザーの指示により見送った
  （コードは一度実装したが、無料翻訳ベースの実装に置き換え済み）。
- 現在の実装：Alpha Vantage NEWS_SENTIMENTが返す`summary`（英語・事実ベースの要約）を
  `translate_to_japanese()`（deep-translator、Google翻訳の無料・非公式エンドポイント）で
  日本語化し、最初の1文だけを一行要約として表示する。煽り度は控えめ（事実の要約止まり）。
  翻訳失敗時（非公式APIのためレート制限等で失敗しうる）はテイザー行を省略するだけで、
  記事生成自体は止めない。
...
**古いファイルのoutput/old/への自動整理（2026-09-23追加）**：記事下書き・サムネイル・テーブル画像・
米国株ピック下書きがoutput/直下に溜まり続けるとの指摘を受け、
[src/formatters/archive_old_files.py](src/formatters/archive_old_files.py)を追加した。
`main.py`のステップ9/9として、実行のたびにファイル名の日付を見て7日より古いものを
`output/old/`へ**移動**する（削除ではない）。削除ではなく移動にしたのは、note.comへの投稿が
今のところ手動（コピー＆ペースト）であり、システム側は「投稿済みかどうか」を把握できないため、
削除だと生成が投稿より先行した場合（Task Schedulerが数日起動しなかった実例あり）に
未投稿の記事を誤って失うリスクがあるとユーザーに説明し、移動方式で合意した。
また、「週の始めに」という運用イメージだったが、特定の曜日にしか整理しない実装だと
